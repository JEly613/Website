...
export function SectionCard({ title, description, href }: SectionCardProps) {
  return (
    <Link href={href} className="block">
      <motion.article
        className="max-w-md overflow-hidden rounded-sharp bg-text border border-border/30 px-10 py-9 shadow-2xl"
        whileHover={{ scale: 1.02, y: -2 }}
        transition={spring.snappy}
      >
        <h2 className="font-display text-5xl font-bold tracking-tight text-bg sm:text-6xl">
        ...
        <span className="mt-6 inline-flex items-center gap-2 text-sm font-medium uppercase tracking-widest text-accent">
          <span className="flex h-8 w-8 items-center justify-center rounded-sharp border border-accent/60 bg-accent/10">
